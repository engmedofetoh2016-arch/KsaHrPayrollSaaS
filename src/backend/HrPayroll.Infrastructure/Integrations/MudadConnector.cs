...
public sealed class MudadConnector : IGovernmentConnector
{
    private readonly IHttpClientFactory _httpClientFactory;
    private readonly IOptions<MudadOptions> _options;
    private readonly ILogger<MudadConnector> _logger;

    public MudadConnector(
        IHttpClientFactory httpClientFactory,
        IOptions<MudadOptions> options,
        ILogger<MudadConnector> logger)
    {
    ...
    public async Task<GovernmentSyncResult> SyncAsync(GovernmentSyncRequest request, CancellationToken cancellationToken)
    {
        ...
        if (!options.Enabled || string.IsNullOrWhiteSpace(options.BaseUrl) || string.IsNullOrWhiteSpace(options.ApiKey))
        {
            return new GovernmentSyncResult(
                ...
                null,
                "{}",
                "Mudad connector is not configured (enabled/baseUrl/apiKey).");
        }

        try
        {
            var client = _httpClientFactory.CreateClient("MudadConnector");
            ...
            if (!response.IsSuccessStatusCode)
            {
                return new GovernmentSyncResult(
                    ...
                    $"Mudad returned {(int)response.StatusCode}.");
            }
        ...
        {
            _logger.LogWarning(ex, "Mudad sync failed for operation {Operation}", request.Operation);
